...
class WrsMainController():
    ...
    """
    IGNORE_LIST = ["small_marker", "large_marker", "lego_duplo",
                   "spatula", "nine_hole_peg_test", "clamp"]  # clampを追加
    ...
    def __init__(self):
        ...
        self.instruction_sub = rospy.Subscriber("/message", String,
                                                self.instruction_cb, queue_size=10)
        self.detection_sub = rospy.Subscriber("/detect_msg", String,
                                              self.detection_cb, queue_size=10)
    ...
    def get_most_graspable_obj(cls, obj_list):
        ...
        ignore_str = ""
        for obj in obj_list:
            info_str = (f"{obj.label:<15}({obj.score:.2%}, "
        ...
        for obj_info in sorted(extracted, key=lambda x: x["score"], reverse=True):
            obj = obj_info["bbox"]
            info_str = (f"{obj.label} ({obj.score:.2%}, "
                        f"{obj.x:3d}, {obj.y:3d}, {obj.w:3d}, {obj.h:3d})\n")
    ...
    def extract_target_obj_and_person(instruction):
        ...
        parts = instruction.split(" to person ")
        target_obj = parts[0].strip()
        target_person = parts[1].strip()
    ...
    def grasp_from_side(self, pos_x, pos_y, pos_z, yaw, pitch, roll, preliminary="-y"):
        ...
        rospy.loginfo(f"move hand to grasp ({pos_x:.2f},{pos_y:.2f}, {pos_z:.2f})")
        ...
        whole_body.move_end_effector_pose(grasp_back["x"], grasp_back["y"],
                                          grasp_back["z"], yaw, pitch, roll)
        whole_body.move_end_effector_pose(grasp_pos["x"], grasp_pos["y"],
                                          grasp_pos["z"], yaw, pitch, roll)
        gripper.command(0)
        whole_body.move_end_effector_pose(grasp_back_safe["x"], grasp_back_safe["y"],
                                          grasp_back_safe["z"], yaw, pitch, roll)
    ...
    def get_place(self, label):
        ...
        kitchen_items = ["windex_bottle", "bleach_cleanser", "sponge", "pitcher_base",
                         "pitcher_lid", "plate", "bowl", "fork", "spoon", "spatula",
                         "wine_glass", "mug"]

        shape_items = ["credit_card_blank", "mini_soccer_ball", "softball", "baseball",
                       "tennis_ball", "racquetball", "golf_ball", "marble", "cup", "foam_brick",
                       "dice", "rope", "chain"]

        tools = ["large_marker", "small_marker", "padlock", "bolt_and_nut", "clamp"]

        task_items = ["rubiks_cube", "colored_wood_block", "nine_hole_peg_test", "toy_airplane",
                      "lego_duplo", "magazine", "black_t_shirt", "timer"]
    ...
    def execute_task1(self):
        ...
        item_dict = {"cracker_box": "tray_a_place", "tuna_fish_can": "container_a_place_1",
                     "potted_meat_can": "bin_a_place", "cup": "drawer_in_left",
                     "foam_brick": "bin_a_place", "toy_airplane": "container_a_place_2",
                     "windex_bottle": "tray_a_place"}
        hsr_position = [
        ...
        for plc, pose in hsr_position:
            ...
            while True:
                ...
                # 指定場所に置く
                # 汎用性有。seed86で使用。
                # self.put_in_place(self.get_place(label), self.get_put_pose(label))

                # 汎用性無。seed1で使用。
                self.put_in_place(item_dict.get(label, "container_a_place"),
                                  self.get_put_pose(label))
                count += 1
                if count >= 7:
                    break
        self.close_drawers()
